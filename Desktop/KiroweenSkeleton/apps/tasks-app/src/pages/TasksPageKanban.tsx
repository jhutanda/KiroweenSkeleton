...
const TasksPageKanban: React.FC = () => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeTask && columns.some(col => col.id === overColumn)) {
      if (activeTask.status !== overColumn) {
        try {
          ...
          dispatch(addNotification({
            type: 'success',
            message: `Task moved to ${columns.find(c => c.id === overColumn)?.title}`,
          }));
        } catch (error) {
          dispatch(addNotification({
            type: 'error',
            ...
            {columns.map((column) => {
              ...
              return (
                <div key={column.id} className="flex-1 min-w-[320px] flex flex-col">
                  {/* Column Header */}
                  <div className={`${column.color} rounded-t-lg px-4 py-3 border-b-2 ${
                    column.id === 'pending' ? 'border-gray-300 dark:border-gray-600' :
                    column.id === 'in-progress' ? 'border-blue-400 dark:border-blue-600' :
                  ...
                  <SortableContext items={tasks.map(t => t.id)}>
                    <div
                      className={`flex-1 ${column.color} rounded-b-lg p-4 space-y-3 overflow-y-auto min-h-[400px]`}
                      id={column.id}
                    >
                      {tasks.length === 0 ? (
...
export default TasksPageKanban;